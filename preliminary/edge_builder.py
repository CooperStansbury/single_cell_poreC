...
def load_alignments_from_csv(filepath):
    ...
    NOTES:
        (1) the snakemake pipeline is here: https://github.com/nanoporetech/Pore-C-Snakemake. 
        We are working with the alignment table (.parquet) that comes from Pore-C-Snakemake/rules/mapping.smk
...
def get_maximal_reads(df, n=2):
    ...
    grped = grped.sort_values(by=['n_fragments', 'perc_of_alignment'], ascending=False)
    ...
    mask = (grped['matching_group_max'] == grped['n_fragments']) & (grped['N_TOP'] <= n)
    grped['SELECT'] = np.where(mask, 1, 0)
    
    read_ids = grped[grped['SELECT'] == 1]['read_idx']
...
def build_paohvis_output(df):
    ...
    """
    pao_df = df[['read_idx', 'fragment_mean', 'chrom']].reset_index(drop=True)
    pao_df['slot_1'] = ""
    pao_df['slot_2'] = ""
    pao_df = pao_df[['read_idx', 'fragment_mean', 'slot_1', 'slot_2', 'chrom']]
    return pao_df 
